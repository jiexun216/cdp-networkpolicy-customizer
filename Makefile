...
# Image URL to use all building/pushing image targets cdp-cdp-networkpolicy-customizer
...
image/webhook-server: $(shell find . -name '*.go')
	CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o cdp-networkpolicy-customizer

# Build the docker image
docker-build: image/webhook-server
	docker build --no-cache . -t $(IMAGE)
	rm -rf cdp-networkpolicy-customizer
